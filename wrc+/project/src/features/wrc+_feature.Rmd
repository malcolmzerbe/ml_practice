---
title: "Predict wRC+ Feature Script"
date: "7/24/2022"
...
```{r}
# load in data
batting2019_tbl <- read.csv('./project/volume/data/raw/batting2019.csv')
batting2019_tbl <- as_tibble(batting2019_tbl)

batting2018_tbl <- read.csv('./project/volume/data/raw/batting2018.csv')
batting2018_tbl <- as_tibble(batting2018_tbl)

batting2017_tbl <- read.csv('./project/volume/data/raw/batting2017.csv')
batting2017_tbl <- as_tibble(batting2017_tbl)

batting2016_tbl <- read.csv('./project/volume/data/raw/batting2016.csv')
batting2016_tbl <- as_tibble(batting2016_tbl)
```

```{r}
# join 2018 features
wrc_tbl <- left_join(batting2019_tbl, batting2018_tbl, by = "playerid")
#wrc_tbl <- wrc_tbl %>%
           #relocate(FullSeason_OBP, .after = HardHit._2018)
```

```{r}
colnames(wrc_tbl) = gsub(".x", "_2019", colnames(wrc_tbl))
colnames(wrc_tbl) = gsub(".y", "_2018", colnames(wrc_tbl))
glimpse(wrc_tbl)
```
